...
export default function App() {
  ...
  useEffect(() => {
    const checkReminders = () => {
      ...
      todos.forEach(todo => {
        if (todo.time === currentTime && !todo.is_completed && !notifiedTasks.has(todo.id)) {
          // 1. Play Alarm Sound
          alarmAudio.current.play().catch(e => console.log("Audio play blocked"));
          
          // 2. Show Push Notification via Service Worker
    ...
    const interval = setInterval(checkReminders, 15000); // Cek tiap 15 detik
  ...
  const handleStart = async () => {
    ...
    // Memancing audio agar diizinkan oleh browser
    ...
    <div className="w-full h-screen bg-white flex flex-col relative overflow-hidden font-sans">
      ...
      <main className="flex-1 overflow-y-auto px-5 py-2 pb-32">
        <div className="mb-8">
          ...
          <p className="text-[#009B7D] font-bold text-sm tracking-wide uppercase mt-1">
        ...
        <div className="space-y-4">
          {todos.length === 0 ? (
            <div className="text-center py-20 text-gray-300">Belum ada rencana hari ini.</div>
          ) : (
            todos.map(todo => (
              <div 
                key={todo.id} 
              ...
              >
                <div className="flex items-center gap-4 flex-1">
                  <button onClick={(e) => { e.stopPropagation(); toggleComplete(todo); }} className="relative">
                    {todo.is_completed ? (
                ...
                <button onClick={(e) => handleDelete(e, todo.id)} className="p-2 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <button 
        ...
        className="fixed bottom-10 right-8 w-16 h-16 bg-primary rounded-full flex items-center justify-center text-white shadow-[0_15px_30px_rgba(0,155,125,0.4)] active:scale-90 transition-all z-40"
      >
